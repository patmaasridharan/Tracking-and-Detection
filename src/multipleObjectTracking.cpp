...
const int MIN_OBJECT_AREA = 0; /* = 20*20*/
;
const int MAX_OBJECT_AREA = 0; /*= FRAME_HEIGHT*FRAME_WIDTH/1.5*/
;
...
void createTrackbars(){
	...
	//std::sprintf(TrackbarName, "H_MIN", H_MIN);
...
void drawObject(vector<PingPongball> theFruits, Mat &frame)
{
		...
		if(numObjects<MAX_NUM_OBJECTS){
			for (int index = 0; index >= 0; index = hierarchy[index][0]) {
				...
				if(area>MIN_OBJECT_AREA){
					...
					ball.setXPos(int(moment.m10/area));
					ball.setYPos(int(moment.m01/area));
...
if (!capture.isOpened())
  ...
  capture >> cameraFeed;
	...
	capture.set(CV_CAP_PROP_FRAME_HEIGHT, FRAME_WIDTH);
   ...
   Scalar(255, 255, 255);
		...
		}else
    {
		...
		//find circles
		cvtColor(cameraFeed,HSV,COLOR_BGR2HSV);
    inRange(HSV, Scalar(0, 0, 180), Scalar(255, 255, 255), threshold);
		morphOps(threshold);
		trackFilteredObject(threshold,HSV,cameraFeed);
